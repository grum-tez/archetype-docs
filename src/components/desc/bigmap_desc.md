...
:::info
The constraint is that a big map is **not** iterable (because only a *hash* of the key is stored, not the key itself). It means that [`for`](/docs/reference/instructions#for), [`while`](/docs/reference/instructions#while), [`iter`](/docs/reference/instructions#iter) and [`length`](/docs/reference/expressions/builtins#length(o%20:%20T)) instructions are not available for big maps.
:::