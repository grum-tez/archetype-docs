(function (Prism) {
	...
	Prism.languages.archetype = {
    	...
    	'builtin': /\b(?:create_contract|make_asset|put_remove|to_container|put|make_map|make_list|blake2b|key_to_address|check_signature|int_to_nat|call_view|slice|mutez_to_nat|fold|get_entrypoint|reverse|require_entrypoint|exec_lambda|lambda_michelson|apply_lambda|get_some|is_some|left|right|some|none|isempty|length|put|get|transfer|call|emit|prepend|make_operation)\b/,
    	...
    	'asset': /\b(?:update_all|clear|count|sum|asc|desc|remove_if|remove_aggregate|remove|update|contains|remove_all|select|sort|head|tail|nth|add|put|add_update|get)\b/,
		'access': /(?:\[|\]|\?|::)/,
