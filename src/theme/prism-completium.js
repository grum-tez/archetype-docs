(function (Prism) {
	...
	Prism.languages.completium = {
		...
		'first': /\b(?:create-react-app|show endpoint|switch endpoint|switch account|set endpoint|start sandbox|stop sandbox|call|deploy|help|init|generate account|accounts|clone|create project|mockup|generate|set binary path|switch mode|install|set mode|show mode|run|binding-ts|binding-dapp-ts|container ls|import faucet|transfer|remove endpoint|import privatekey|show account|set account|remove account|originate|show entries|show storage|show contract)\b/,
		'checked': /\b(?:✔)\b/
	};
}(Prism));
