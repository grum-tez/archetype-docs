(function (Prism) {
	...
	Prism.languages.completium = {
		...
		'top' : /^((\$ )?completium-cli)|^((\$ )?npm)|^((\$ )?cd)|^(git)|^(opam)|^(make)|^(eval)|^(npx)|^(\$ )?docker|^((\$ )?cat)|^((\$ )?octez-client)/m,
		'first': /\b(?:--with-dapp-originate|create-react-app|register global constant|show binary path|show endpoint|switch endpoint|switch account|show address|set endpoint|start sandbox|stop sandbox|call|deploy|help|init|generate account as|accounts|clone|create project|mockup|generate|set binary path|switch mode|install|set mode|show mode|run|binding-ts|binding-dapp-ts|container ls|import faucet|transfer|remove endpoint|import privatekey|show account|set account|remove account|originate|show entries|show storage|show contract)\b/,
		'checked': /\b(?:✔)\b/
	};
}(Prism));
